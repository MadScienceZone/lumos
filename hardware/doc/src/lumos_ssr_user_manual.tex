...
%\vfill
\begin{center}
{\fontfamily{pag}\fontseries{b}\fontsize{64}{80}\selectfont
...
a value greater than 127 needs to be sent, there is an escape mechanism recognized by the Lumos
controller.  The following two byte values are special:
\begin{description}
...
\strong{The following is obsolete:}
\begin{BF}
  \bitbox{1}{\z1} & \bitbox{3}{3} & \bitbox{4}{\Var*{addr}}\\
...
In more detail, each half-wave period (1/120\,s for the 60\,Hz power frequency standard
...
For the main intended application of these controllers---incandescent lamps for the AC
boards and \acronym{LED}s for the DC boards---this produces a visual effect of the light
...
controller.\footnote{You might decide it's an acceptable risk to attach one of these loads, such as a
``non-dimmable'' \acronym{CFL} light, if you only tell the Lumos board to turn it on
or off, and never use the dimmer settings.  In theory, that may be ok, all other things
...
or a glitch in communications won't be misinterpreted as such, or that even a firmware
bug on the controller won't cause this. It's an informed risk you can decide to take, but
we don't officially recommend it.}
...
There is an obscure device setting called ``\ix{phase offset}'' on the Lumos controllers which controls
the delay between the receipt of the zero-crossing signal and the time of the actual zero-crossing
...
that is no longer the case.  In practice, there shouldn't be any phase difference between the controller's
...
We will now describe how the phase offset works in the internal timing chain implemented by the Lumos
controller firmware, in case you should find yourself in some strange circumstance where you need
to change this setting.  Otherwise, the normal setting for this value is 2.
...
channels are turned on in order for each of them to be effectively generating the desired \acronym{PWM}
duty cycle.

Assuming that the interrupt arrives at the actual zero-crossing event, as it ideally does,
...
\begin{figure}
 ...
 \def\y{1.5}
  \light{Y2}{sss} \node [right] at (1,\y) {super-slow fading up/down};
 \def\y{1}
  \light{Y2}{x} \node [right] at (1,\y) {not involved/affected; may have any value};
 \end{tikzpicture}
...
This chapter provides the documentation for the \z{lumosasm} and \z{lumosctl} commands.  
