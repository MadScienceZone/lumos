...
;@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
;@@                                                                         @@
;@@  @@@   @   @   @@@   @@@@@  @          24-Channel SSR Controller Board  @@
;@@ @   @  @   @  @   @    @    @          Firmware Version 2.0.2           @@
;@@    @   @   @  @        @    @                                           @@
;@@   @    @@@@@  @        @    @          For use with controller hardware @@
;@@  @         @  @        @    @          PROTOTYPE-1  (handcrafted boards @@
;@@ @   @      @  @   @    @    @          with PIC777 processor and 24 SSR @@
;@@ @@@@@      @   @@@     @    @@@@@      units integrated)                @@
;@@                                                                         @@
;@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
;
...
;
; $Header: /usr/local/cvsroot/ssr/firmware/24ctlprotorom.asm,v 1.1 2007/12/18 07:12:48 steve Exp $
;
; Adapted from 48-channel control ROM firmware 2.0.2 "48ctlrom.asm", to update
; the original prototype board with firmware improvements from this version.
;
; Target Microcontroller is PIC16F777, Q=20MHz
;  (200nS instruction cycle) 
; Serial control (RS-485) at 19.2kbps
;
; Since this is a 777 controller, we can't store EEPROM values.
; The device ID is therefore *hard-coded* in the ROM firmware.
;
;=============================================================================
; HARDWARE DESCRIPTION
;-----------------------------------------------------------------------------
;
;  PIC16F777 Microcontroller I/O pin assignments:  (production 48CTL usage)
;                        ________   _________
;                       |o       \_/         |
;     Vpp /RESET -->  1 | /MCLR      PGD/RB7 | 40 <-> /SSR15 + PGD
;(/SSR16) /SSR14 <--  2 | RA0        PGC/RB6 | 39 <-> /SSR16 + PGC
;(/SSR14) /SSR13 <--  3 | RA1            RB5 | 38 --> /SSR17 (/SSR15)
;         /SSR12 <--  4 | RA2            RB4 | 37 --> /SSR18 (/SSR13)
;(/SSR10) /SSR11 <--  5 | RA3            RB3 | 36 --> /SSR19 (/SSR11)
;(/SSR08) /SSR10 <--  6 | RA4 (OD)       RB2 | 35 --> /SSR20 (/SSR09)
;   (ACT) /SSR09 <--  7 | RA5            RB1 | 34 --> /SSR21 (/SSR07)
;   (GRN) /SSR08 <--  8 | RE0        INT/RB0 | 33 <-- ZC INT
;   (YEL) /SSR07 <--  9 | RE1    PIC     VDD | 32 --- +5V       
;   (RED) /SSR06 <-- 10 | RE2    16F     VSS | 31 --- GND      
;            +5V --- 11 | VDD    877A    RD7 | 30 --> /SSR22 (/SSR17)
;            GND --- 12 | VSS     or     RD6 | 29 --> /SSR23 (/SSR06)
;           xtal --- 13 | OSC    777     RD5 | 28 --> ACT LED (/SSR05)
;             +----- 14 | OSC            RD4 | 27 --> RED LED (/SSR18)
;(/SSR04) /SSR05 <-- 15 | RC0         RX/RC7 | 26 <-- Serial RxD
;(/SSR19) /SSR04 <-- 16 | RC1         TX/RC6 | 25 --> Serial TxD
;(/SSR03) /SSR03 <-- 17 | RC2            RC5 | 24 --> T/R (/SSR02)
;(/SSR01) /SSR02 <-- 18 | RC3            RC4 | 23 <-- M/S (/SSR00)
;(/SSR23) /SSR01 <-- 19 | RD0            RD3 | 22 --> YEL LED (/SSR22)
;(/SSR21) /SSR00 <-- 20 | RD1            RD2 | 21 --> GRN LED (/SSR20)
...
;
;          7   6   5   4   3   2   1   0
; PORT RA --- --- /09 /10 /11 /12 /13 /14
;          <OSC>   O   O   O   O   O   O
;
; PORT RB /15 /16 /17 /18 /19 /20 /21 ---
;          <PGM>   O   O   O   O   O  INT
;                   _   _
; PORT RC --- --- T/R M/S /02 /03 /04 /05
;          <I/O>   O   I   O   O   O   O
;
; PORT RD /22 /23 ACT RED YEL GRN /00 /01
;          O   O   O   O   O   O   O   O
;
; PORT RE --- --- --- --- --- /06 /07 /08
;                              O   O   O
...
;   (bank 2 too)     |                                                       |
;                    |______|______|______|______|______|______|______|______|
; $005 PORTA         |/////////////| _____| _____| _____| _____| _____| _____|
;                    |/////////////| SSR09| SSR10| SSR11| SSR12| SSR13| SSR14|
;                    |/////////////|______|______|______|______|______|______|
; $006 PORTB         | _____| _____| _____| _____| _____| _____| _____|//////|
;   (bank 2 too)     | SSR15| SSR16| SSR17| SSR18| SSR19| SSR20| SSR21|//////|
;                    |______|______|______|______|______|______|______|//////|
; $007 PORTC         |/////////////|   _  |   _  | _____| _____| _____| _____|
;                    |/////////////| T/R  | M/S  | SSR02| SSR03| SSR04| SSR05|
;                    |/////////////|______|______|______|______|______|______|
; $008 PORTD         | _____| _____|      |      |      |      | _____| _____|
;                    | SSR22| SSR23|  ACT |  RED |  YEL |  GRN | SSR00| SSR01|
;                    |______|______|______|______|______|______|______|______|
; $009 PORTE         |//////////////////////////////////| _____| _____| _____|
;                    |//////////////////////////////////| SSR06| SSR07| SSR08|
...
;                    |SSR_ON|SSRDIM|      | Dim value (0=off .. 31=full on)  |
;                    |______|______|______|______|______|______|______|______|
; $038 PORTA_BUF     |/////////////| _____| _____| _____| _____| _____| _____|
;                    |/////////////| SSR09| SSR10| SSR11| SSR12| SSR13| SSR14|
;                    |/////////////|______|______|______|______|______|______|
; $039 PORTB_BUF     | _____| _____| _____| _____| _____| _____| _____|//////|
;                    | SSR15| SSR16| SSR17| SSR18| SSR19| SSR20| SSR21|//////|
;                    |______|______|______|______|______|______|______|//////|
; $03A PORTC_BUF     |/////////////|   _  |   _  | _____| _____| _____| _____|
;                    |/////////////| T/R  | M/S  | SSR02| SSR03| SSR04| SSR05|
;                    |/////////////|______|______|______|______|______|______|
; $03B PORTD_BUF     | _____| _____|      |      |      |      | _____| _____|
;                    | SSR22| SSR23|  ACT |  RED |  YEL |  GRN | SSR00| SSR01|
;                    |______|______|______|______|______|______|______|______|
; $03C PORTE_BUF     |//////////////////////////////////| _____| _____| _____|
;                    |//////////////////////////////////| SSR06| SSR07| SSR08|
;                    |//////////////////////////////////|______|______|______|
; $03D PHASE_OFFSET  |                                                       |
...
;                    |______|______|______|______|______|______|______|______|
; $106 PORTB         | _____| _____| _____| _____| _____| _____| _____|//////|
;   (bank 0 too)     | SSR15| SSR16| SSR17| SSR18| SSR19| SSR20| SSR21|//////|
;                    |______|______|______|______|______|______|______|//////|
; $10C EEDATA        |                                                       |
...
;   (ZC)                                (ZC)
;
; *** PROTOTYPE BOARD VERSION: We *do* have the phase shift as described above ***
...
;==============================================================================
;
; Hard-coded things we don't have EEPROM for in a '777 chip
;
HARDCODED_DEVICE_ID EQU	.10
HARDCODED_PHASE_OFF EQU	.2
;
...
;                     ___7______6______5______4______3______2______1______0__
; $005 PORTA         |/////////////| _____| _____| _____| _____| _____| _____|
; $038 PORTA_BUF     |/////////////| SSR09| SSR10| SSR11| SSR12| SSR13| SSR14|
;                    |/////////////|______|______|______|______|______|______|
;
PORTA_BUF	EQU	0x038
...
PBUF_09		EQU	PORTA_BUF
PBUF_10		EQU	PORTA_BUF
PBUF_11		EQU	PORTA_BUF
PBUF_12		EQU	PORTA_BUF
PBUF_13		EQU	PORTA_BUF
...
;                     ___7______6______5______4______3______2______1______0__
; $006 PORTB         | _____| _____| _____| _____| _____| _____| _____|//////|
; $039 PORTB_BUF     | SSR15| SSR16| SSR17| SSR18| SSR19| SSR20| SSR21|//////|
;                    |______|______|______|______|______|______|______|//////|
;
PORTB_BUF	EQU	0x039
...
PBUF_16		EQU	PORTB_BUF
PBUF_17		EQU	PORTB_BUF
PBUF_18		EQU	PORTB_BUF
PBUF_19		EQU	PORTB_BUF
PBUF_20		EQU	PORTB_BUF
PBUF_21		EQU	PORTB_BUF
...
;                     ___7______6______5______4______3______2______1______0__
; $007 PORTC         |/////////////|   _  |   _  | _____| _____| _____| _____|
; $03A PORTC_BUF     |/////////////| T/R  | M/S  | SSR02| SSR03| SSR04| SSR05|
;                    |/////////////|______|______|______|______|______|______|
;
PORTC_BUF	EQU	0x03A

PORT_TRSEL	EQU	PORTC		; Transmit/receive select (keep at 0)
PBUF_TRSEL	EQU	PORTC_BUF
BIT_TRSEL	EQU	5

PORT_MSDET	EQU	PORTC		; (unused) Master/Slave detect
PBUF_MSDET	EQU	PORTC_BUF
BIT_MSDET	EQU	4
...
;                     ___7______6______5______4______3______2______1______0__
; $008 PORTD         | _____| _____|      |      |      |      | _____| _____|
; $03D PORTD_BUF     | SSR22| SSR23| ACT  |  RED |  YEL |  GRN | SSR00| SSR01|
;                    |______|______|______|______|______|______|______|______|
;
PORTD_BUF	EQU	0x03B
...
PORT_LEDS	EQU	PORTD
;
; These masks are used during initial device setup ONLY
; and should reflect the ENTIRE port state if ONLY these
; LEDs are lit.  Therefore, note the /SSRxx lines set to
; 1 in all cases, so the SSRs are OFF while we fiddle with
; the LEDs during bootup.
;
MASK_GRN_YEL	EQU	b'11001111'	; RED off; YEL, GRN on
MASK_RED_YEL	EQU	b'11011011'	; RED, YEL on; GRN off
MASK_ALL_LEDS	EQU	b'11011111'	; RED, YEL, GRN on
MASK_NO_LEDS	EQU	b'11000011'	; all off

PORT_ACT	EQU	PORTD		; This is carried to the front panel
PBUF_ACT	EQU	PORTD_BUF 	; but unused.  need to place an LED 
BIT_ACT		EQU	5		; there.
...
PBUF_RED	EQU	PORTD_BUF
PBUF_YEL	EQU	PORTD_BUF
PBUF_GRN	EQU	PORTD_BUF
...
PBUF_00		EQU	PORTD_BUF
PBUF_01		EQU	PORTD_BUF
...
;                     ___7______6______5______4______3______2______1______0__
; $009 PORTE         |//////////////////////////////////| _____| _____| _____|
; $03C PORTE_BUF     |//////////////////////////////////| SSR06| SSR07| SSR08|
;                    |//////////////////////////////////|______|______|______|
;
PORTE_BUF	EQU	0x03C
...
PBUF_06		EQU	PORTE_BUF
PBUF_07		EQU	PORTE_BUF
PBUF_08		EQU	PORTE_BUF
...
SETUP_PORTS	CLRWDT
		...
		MOVWF	TRISA                   
		MOVLW	b'00000001'		; OOOOOOOI
		MOVWF	TRISB
		MOVLW	b'11010000'		; XXOIOOOO
...
;
		BSF	PORT_GRN, BIT_GRN	; @@P=877@@
		NOP                  		; @@P=877@@
		MOVLW	MASK_GRN_YEL		; @@P=777@@
...
SETUP_DEV_ID	CLRWDT
		...
		CLRF	EEDATH		; @@P=877@@ Clear high bits of data...
		CLRF	EEADRH		; @@P=877@@ ...and address
		MOVLW	EE_DEV_ID	; @@P=877@@ Set target EEPROM location
		MOVWF	EEADR		; @@P=877@@ 
		BANKSEL	EECON1		; @@P=877@@ (Bank 3)
		BCF	EECON1, EEPGD	; @@P=877@@ Select EEPROM data memory
		BSF	EECON1, RD	; @@P=877@@ Start read operation
		BANKSEL	EEDATA		; @@P=877@@ (Bank 2)
		MOVF	EEDATA, W	; @@P=877@@ W = device ID value
		MOVLW	HARDCODED_DEVICE_ID ; @@P=777@@
...
SETUP_PHASE	CLRWDT
		BANKSEL	EEADR		; @@P=877@@ (Bank 2)
		MOVLW	EE_PHASE	; @@P=877@@ Set target EEPROM location
		MOVWF	EEADR		; @@P=877@@
		BANKSEL	EECON1		; @@P=877@@ (Bank 3)
		BCF	EECON1, EEPGD	; @@P=877@@ Select EEPROM data memory
		BSF	EECON1, RD	; @@P=877@@ Start read operation
		BANKSEL	EEDATA		; @@P=877@@ (Bank 2)
		MOVF	EEDATA, W	; @@P=877@@ W = device ID value
		MOVLW	HARDCODED_PHASE_OFF ; @@P=777@@
...
PASS_CMD_1	CLRWDT				; not my SSR, send to slave CPU
		GOTO	FAULT			; no slave in 24SSR controller!
...
CMD_SET_PHASE	CLRWDT
		GOTO	CMD_ERROR		; not implemented in prototype
;
...
; $Log: 24ctlprotorom.asm,v $
; Revision 1.1  2007/12/18 07:12:48  steve
