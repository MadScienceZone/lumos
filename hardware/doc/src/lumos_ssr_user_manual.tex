...
\vfill
\begin{center}
{\HUGE Using the Lumos\TM}

\bigskip

{\HUGE  SSR Controllers}
...
normal \acronym{RAM} memory, clears all faults and error conditions, and re-initialized all the hardware
...
referred to as ``privileged mode''.} to prevent potentially harmful effects such as accidentally changing
...
this is accomplished by using the \z{lumosctl} command-line program on the host PC.  The basic configuration
...
Looking in the \z{lumos\_board.cfg}, find a section beginning with ``\z{[lumos\_device\_settings]}''.
...
loaded back to the board again with the \z{----load--configuration=}\Var*{file} option:
...
$ lumosctl --dump-configuration=lumos_board.cfg
\end{SourceCode}
In the \z{[lumos\_device\_settings]} section, add a field ``\z{dmxchannel=}\Var*{c}'' where
...
\begin{figure}
  \begin{center}
	...
	\multicolumn{3}{|c|}{Expression} & Meaning\\\hline\hline
...
The call-return stack receives three bytes every time a sequence calls another sequence:
\begin{BF}
...
the top of the stack is the counter for the innermost executing loop.
...
The loop stack would look like:
\begin{BF}
  \wordbox{1}{\Var{x}}\\
  \wordbox{1}{\Var{y}}\\
  \wordbox{1}{\Var{z}}\\
...
stack frame pointer is moved to point to the first parameter.
...
\begin{itemize}
	...
	\item[\Var{i}=] All data values are immediate bytes (\Var{i}=1) instead of being popped
		from the stack (\Var{i}=0).
	\item[\Var{d}=] Take no stack/immediate values; assume default values inherently (\Var{d}=1).
\end{itemize}
...
\begin{itemize}
	\item\emph{Default addressing} means that no data values are provided, 
		and the instruction should assume
		a useful default value instead.  Denoted below as an instruction missing any
		mention of the defaulted arguments.  The preferred notation is to name the value
		as normal, and allow the assembler to optimize it to the default version.
	\item\emph{Direct addressing} specifies a memory location as a constant value following
	...
	\item\emph{Immediate addressing} mode means that the values needed by the instruciton are
	...
	\item\emph{Indirect addressing} is used when a data value used by an operation is
	...
	\item\emph{Indirect Stack} values are obtained by popping an address from the stack and
	...
	\item\emph{Stack addressing} means that the data value is popped from the stack.  These are
...
\begin{bytefield}[leftcurlyspace=0pt, rightcurlyspace=0pt, leftcurly=., rightcurly=.]{16}
	...
	\begin{rightwordgroup}{\z{exit}}
	\bitbox{1}{\z0} & \bitbox{1}{\z0} & \bitbox{1}{\z0} & \bitbox{5}{\z{0x00}}
	\end{rightwordgroup} \\
\end{bytefield}
...
\subsubsection{\z{ON/OFF}: Turn a Channel On Or Off}
\begin{bytefield}[leftcurlyspace=0pt, rightcurlyspace=0pt, leftcurly=., rightcurly=.]{16}
	\bitbox[]{8}{\footnotesize byte 0} &
	\bitbox[]{8}{\footnotesize byte 1} \\

	\begin{rightwordgroup}{\z{off} \Var*{channel}}
	\bitbox{1}{\z0} & \bitbox{1}{\z0} & \bitbox{1}{\z0} & \bitbox{5}{\z{0x01}} & \bitbox{8}{\Var*{channel}}
	\end{rightwordgroup} \\
	\begin{rightwordgroup}{\z{on} \Var*{channel}}
	\bitbox{1}{\z1} & \bitbox{1}{\z0} & \bitbox{1}{\z0} & \bitbox{5}{\z{0x01}} & \bitbox{8}{\Var*{channel}}
	\end{rightwordgroup} \\
\end{bytefield}
...
including source, assembly, binary
...
In the command descriptions that follow, we will show the bytes sent or received graphically, using the
following notation:
\begin{itemize}
...
\begin{BF}
  ...
  \bitbox{1}{\z0} & \bitbox{7}{\Var*{steps}$-1$}\\
  \bitbox{1}{\z0} & \bitbox{7}{\Var*{time}$-1$}
\end{BF}
...
the effective ranges for each of those values is 1--128.
...
\begin{description}
	...
	\item[Sensor information:]
		...
		activates, \Var{w} it it's set to run while the sensor is active, and \Var{e}
		indicates if the sensor is active high (\Var{e}=1) or active low (\Var{e}=0).
...
When the sensor no longer is triggering, \Var*{exec} plays on until it completes, then \Var*{post} is
run once.
...
will change the device's address to 0 and its baud rate to 19,200\,bps.
{\bfseries This command is only recognized if the controller is in configuration mode.}
...
\subsection{Reserved Command Codes}
Codes \z{0x775}--\z{0x77f} are reserved for future use as configuration-mode commands.
...
\chapter{Lumosctl CLI Manual Entry}\label{ch:lumosctl}
The \z{lumosctl} command-line utility is used to configure and program the Lumos controllers.  It can also
be used to control the output states, but not in as fine-tuned or sophisticated a manner as programs
such as \z{lplay} (part of the Lumos light orchestration software suite) or a third-party program
such as Vixen.

This chapter provides the documentation for the \z{lumosctl} command.  This same information is also
