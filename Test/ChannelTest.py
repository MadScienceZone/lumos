...
class ChannelTest (unittest.TestCase):
    ...
    def testCons(self):
        ...
        self.assertRaises(ValueError, Channel, 'id3', DummyController())

    def testInvalidDimmer(self):
        self.assertRaises(DimmerError, Channel, 'id', DummyController(), load=2, dimmer=False, warm=20, power_source=self.power)

    def testResolution(self):
        ch = Channel('ch', DummyController(), load=1, dimmer=True, resolution=32, power_source=self.power)
    ...
    def testLevel(self):
        ch = Channel('ch', DummyController(), load=1, dimmer=True, warm=10, resolution=100, power_source=self.power)
        self.assertEqual(ch.level, None)
        ch.set_off()
    ...
    def testLevel2(self):
        ch = Channel('ch', DummyController(), load=1, dimmer=True, warm=10, resolution=48, power_source=self.power)
        ch.kill()
        self.assertEqual(ch.level, None)
        ch.set_off()
