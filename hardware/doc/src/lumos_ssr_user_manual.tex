...
\begin{BF}
	\bitbox{1}{\z1} & \bitbox{3}{6} & \bitbox{4}{\Var*{addr}}\\
	\bitbox{1}{\z0} & \bitbox{1}{\Var{\strut a}} & \bitbox{1}{\Var{\strut i}}
		& \bitbox{1}{\Var{\strut p}} 
		& \bitbox{1}{\Var{\strut s}} 
		& \bitbox{1}{\color{lightgray}\rule{\width}{\height}}
		& \bitbox{1}{\z{0}}
		& \bitbox{1}{\z0}\\
	\bitbox{1}{\z0} & 
		...
		\bitbox[br]{1}{\z0} \\
\end{BF}
With \Var*{e}=0, this initiates a button scan. 

If \Var*{s}=0, the button states are cleared and
...
Otherwise, if \Var*{s}=1, this command merely re-enables the button scanner so that
...
%		respectively.
		...
		In the 16-byte block reserved for sensor trigger information, the field \Var*{$S_n$} is
		still expected to hold the sensor number, which is \z0 for the first sensor (A),
		\z1 for sensor B, and so on.
...
%			\bitbox[br]{1}{\z0} \\
	...
	controlled by the \acronym{QSMC} unit. (You need to include them,
	...
	(all of which must also have the \Var{m} bit set)
	...
	For another example, if you have, say, three \acronym{QSCC} units followed by a \acronym{QSXT} on the 
...
\subsection{\z{0x710} Clear Quiz Show Displays}
\begin{BF}
	\bitbox{1}{\z1} &
	\bitbox{3}{7} &
	\bitbox{4}{\Var*{addr}} \\
	...
	\bitbox{1}{\z0} & \bitbox{3}{\color{lightgray}\rule{\width}{\height}}&
	\bitbox{1}{\Var{\strut b}} &
	\bitbox{1}{\Var{\strut m}} &
	\bitbox{1}{\Var{\strut d}} &
	\bitbox{1}{\Var{\strut o}}\\
	\bitbox{1}{\z0} &
	\bitbox[tb]{1}{\z0} &
	\bitbox[tb]{1}{\z1} &
	\bitbox[tb]{1}{\z1} &
...
If \Var*{b} is set, the button states are cleared without changing whether the scanner itself
is enabled or running.

If \Var*{m} is set, the readerboard matrix is cleared to black.

If \Var*{d} is set, the \acronym{LED} display is cleared.

If \Var*{o} is set, all output channels are turned off.
\end{QS*}
...
\begin{QS*}
	\z{0x70a}--\z{0x70d} and \z{0x70f}--\z{0x710} are used for QS-specific commands.
