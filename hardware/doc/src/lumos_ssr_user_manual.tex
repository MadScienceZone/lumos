...
\begin{enumerate}
	...
	\item	Install a \ix{jumper} across pins 4--5 of J2.
...
\begin{QS}
Additionally, the \z{BLACKOUT} and \z{SCAN} commands are recognized by all units for
address 15 (i.e., all address bits `\z1').
\end{QS}
	...
	\item	If bit 7 is set:
		\begin{enumerate}
			...
			\item	If \Var*{addr} matches my address, interpret command code \Var*{cmd}
				and act upon it.
			\begin{QS*}
			\item	If \Var*{addr} is \z{0xF}, and the command is \z{BLACKOUT} or \z{SCAN},
				go ahead and act on that.
			\end{QS*}
...
Once the level has reached the full extend of its range, it remains there.  However, 
...
\begin{QS}
\subsection{\z{0x5}: Start Scanning}
\begin{BF}
  \bitbox{1}{\z1} & \bitbox{3}{5} & \bitbox{4}{15}
\end{BF}
Upon receipt of the single byte \z{11011111} (\z{0xDF}), all QS devices will simultaneously
clear their button state timers and start timing a new set of response times.  The
\z{QS\_QUERY} command (\z{0x70a}) is then used to monitor the developing state of responses
and eventually to halt the scanners.  Note that scanners may stop on their own when they 
reach their maximum time values (17 minutes, 53.741824 seconds).
\end{QS}
	...
	\item[Device identity:]
		...
		\begin{center}
		 \begin{tabular}{cl}\toprule
		  ...
		  4 & reserved \begin{QS*}QSRC\end{QS*} \\
		  5 & reserved\\
		  6 & reserved\\
		  7 & reserved\\
		  8 & reserved\\
...
\begin{BF}
  \bitbox{1}{\z1} & \bitbox{3}{7} & \bitbox{4}{\Var*{addr}}\\
  \bitbox{1}{\z0} & \bitbox{7}{29}\\
  \bitbox{1}{\z0} & \bitbox{1}{\strut\Var{s}} & \bitbox{6}{24} \\
  \begin{rightwordgroup}{\(\times 6\): X, L, A, B, C, D}
  \bitbox{1}{\strut\Var{m}} &
	\bitbox{1}{\strut\Var{p}} &
	\bitbox[lrt]{6}{\strut\Var*{time}}\\
	\wordbox[lr]{1}{(30-bit value}\\
	\wordbox[lr]{1}{in $\mu$S)}\\
...
\begin{BF}
  \bitbox{1}{\z1} & \bitbox{3}{7} & \bitbox{4}{\Var*{addr}}\\
  \bitbox{1}{\z0} & \bitbox{7}{29}\\
  \bitbox{1}{\z0} & \bitbox{1}{\strut\Var{s}} & \bitbox{6}{40} \\
...
In the full response, a 30-bit time value is returned, which is the number of microseconds that elapsed from
the time the scanner started until the button was pressed.  The two flag bits have this meaning:
\begin{center}
 \begin{tabular}{ccl}\toprule
...
\bottomrule
 \end{tabular}
\end{center}
At least theoretically, it's possible for a button to be pressed in less than 1\,$\mu$s from
the start time, which would result in the 32-bit value \z{0x40000000}, so don't rely only on the
time value alone.
...
\begin{QS*}
\z{0x71d} is used for the response packet for QS query commands.
\end{QS*}
...
running will cause an automatic temporary masking for \Var*{lockout} tenths
of a second after being released.
\end{QS}
