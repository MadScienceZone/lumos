...
class ConfigurationCanvas (tkinter.Frame):
    ...
    def __init__(self, show, *args, **kwargs):
        ...
        hsb = tkinter.ttk.Scrollbar(self, orient=HORIZONTAL)
...
#        self.update()
    ...
    def ok_to_del_power_source(self, src_obj, force=False):
        if src_obj is not None:
            if src_obj.subordinates:
                tkinter.messagebox.showerror("Can't Delete Power Source", "You cannot delete this power source if it still has other power sources plugged into it.")
                return False

            for controller in list(self.show.controllers.values()):
                if controller.power is src_obj:
                    tkinter.messagebox.showerror("Can't Delete Power Source", "You cannot delete this power source if it still has loads plugged into it.")
                    return False
            ...
            if force:
                return True
            return tkinter.messagebox.askokcancel("Confirm Deletion", "Are you sure you wish to delete power source {0}?".format(src_obj.id), default=tkinter.messagebox.CANCEL)
    ...
    def draw_controllers(self):
        ...
        H_GAP = 5
        V_GAP = 5
        cur_x = max([x[2] for x in list(self._network_locations.values())]+[0]) + H_GAP
        cur_y = 0
    ...
    def ok_to_del_network(self, net_id, force=False):
        if net_id in self.show.networks:
            if self.show.networks[net_id].units:
                tkinter.messagebox.showerror("Can't Delete Network", "You cannot delete this network if it still has controller units {0} plugged into it.".format(self.show.networks[net_id].units))
                return False

            if force:
                return True
            return tkinter.messagebox.askokcancel("Confirm Deletion", "Are you sure you wish to delete network {0}?".format(net_id), default=tkinter.messagebox.CANCEL)
    ...
    def ok_to_del_controller(self, ctl_obj, force=False):
        return force or tkinter.messagebox.askokcancel("Confirm Deletion", "Are you sure you wish to delete controller unit {0}?".format(ctl_obj.id), default=tkinter.messagebox.CANCEL)
...
class PowerSourceEditorDialog(tkinter.ttk.Frame):
    def __init__(self, parent, show_obj, power_source_obj=None, parent_source=None):
        ...
        tkinter.ttk.Label(main, text="ID:").grid(row=0, column=0, sticky=W)
        self.v_id = tkinter.StringVar()
        if parent_source is not None:
            idf=tkinter.ttk.Frame(main)
            tkinter.ttk.Label(idf, text=parent_source.id+'.').grid(row=0, column=0, sticky=E)
            tkinter.ttk.Entry(idf, textvariable=self.v_id, width=10, justify=LEFT, validate='key', validatecommand=self.validate_id).grid(row=0, column=1, sticky=W)
            idf.grid(row=0, column=1, sticky=W)
        else:
            tkinter.ttk.Entry(main, textvariable=self.v_id, width=10, justify=LEFT, validate='key', validatecommand=self.validate_id).grid(row=0, column=1, sticky=W)
            

        tkinter.ttk.Label(main, text="Capacity:").grid(row=1, column=0, sticky=W)
        self.v_cap = tkinter.StringVar()
        capf=tkinter.ttk.Frame(main)
        tkinter.ttk.Entry(capf, textvariable=self.v_cap, width=4, justify=RIGHT, validate='key', validatecommand=self.validate_nn).grid(row=0, column=0, sticky=W)
        tkinter.ttk.Label(capf, text="amps").grid(row=0, column=1, sticky=W)
        ...
        tkinter.ttk.Button(main, text="Remove This Source", state='normal' if power_source_obj is not None else 'disabled', command=self.delete_me).grid(row=0, column=10)
        tkinter.ttk.Button(main, text="Add Subordinate", state='normal' if power_source_obj is not None else 'disabled', command=self.add_child).grid(row=1, column=10)
    ...
    def _save(self):
        ...
        if new_id == '':
            tkinter.messagebox.showerror("Invalid ID", "Each power source must have an ID.")
            return
        ...
        if new_cap in ('', '+', '.', '+.') or float(new_cap) <= 0:
            tkinter.messagebox.showerror("Invalid Capacity", "Each power source must supply at least 1 amp.")
            return
        ...
        if not self._is_id_unique(new_id, self.power_source_obj, [self.show_obj.all_power_sources[i] for i in self.show_obj.top_power_sources]):
            tkinter.messagebox.showerror("Duplicate ID", 'The ID "{0}" is already assigned to another power source.'.format(new_id))
            return
...
def _introspect_fields(form, frame, obj, typemap, skip=None, method=None):
    ...
    for attribute_name in inspect.getargspec(method)[0]:
        ...
        if attribute_name == "port":
            ...
            # GUI interface.
            f = tkinter.StringVar()
            fields[attribute_name] = (f, tkinter.ttk.Entry(frame, textvariable=f, width=15, justify=LEFT), str)
        elif type(v) is bool:
            f = tkinter.IntVar()
            fields[attribute_name] = (f, tkinter.ttk.Checkbutton(frame, variable=f), bool)
        elif type(v) is float:
            f = tkinter.StringVar()
            fields[attribute_name] = (f, tkinter.ttk.Entry(frame, textvariable=f, width=10, justify=RIGHT, validate='key', validatecommand=form.validate_float), float)
        elif type(v) is int:
            f = tkinter.StringVar()
            fields[attribute_name] = (f, tkinter.ttk.Entry(frame, textvariable=f, width=10, justify=RIGHT, validate='key', validatecommand=form.validate_int), int)
        else:
            f = tkinter.StringVar()
            fields[attribute_name] = (f, tkinter.ttk.Entry(frame, textvariable=f, width=10, justify=LEFT), str)
...
class NetworkEditorDialog(tkinter.ttk.Frame):
    def __init__(self, parent, show_obj, network_type=None, network_id=None):
        ...
        tkinter.ttk.Label(main, text="{0} Network ID:".format(self.field_spec['_type'])).grid(row=0, column=0, sticky=W)
        self.v_id = tkinter.StringVar()
        tkinter.ttk.Entry(main, textvariable=self.v_id, width=10, justify=LEFT, validate='key', validatecommand=self.validate_id).grid(row=0, column=1, sticky=W)

        for row, field_name in enumerate([n for n in sorted(self.field_spec) if not n.startswith('_')]):
            tkinter.ttk.Label(main, text=field_name+":").grid(row=row+1, column=0, sticky=W)
            print(field_name)
        ...
        tkinter.ttk.Button(main, text="Remove This Network", state='normal' if self.network_obj is not None else 'disabled', command=self.delete_me).grid(row=0, column=10)
        self.pack(side=TOP, expand=True, fill=BOTH)
    ...
    def _save(self):
        ...
        if new_id == '':
            tkinter.messagebox.showerror("Invalid ID", "Each network must have an ID.")
            return

        #new_id = self.v_id.get()
        if new_id in self.show_obj.networks and self.show_obj.networks[new_id] is not self.network_obj:
            tkinter.messagebox.showerror("Duplicate ID", 'The ID "{0}" is already assigned to another network.'.format(new_id))
            return
        ...
        if self.network_obj is None:
            try:
                ...
                tkinter.messagebox.showerror("Error", "Can't create that network: {0}".format(err))
                return
            else:
            ...
            try:
                ...
                tkinter.messagebox.showerror("Error in network values", "{0}".format(err))
                return
...
class ControllerEditorDialog(tkinter.ttk.Frame):
    def __init__(self, parent, show_obj, ctl_type=None, ctl_obj=None):
        ...
        tkinter.ttk.Label(main, text="{0} Controller ID:".format(self.field_spec['_type'])).grid(row=0, column=0, sticky=W)
        self.v_id = tkinter.StringVar()
        tkinter.ttk.Entry(main, textvariable=self.v_id, width=10, justify=LEFT, validate='key', validatecommand=self.validate_id).grid(row=0, column=1, sticky=W)

        tkinter.ttk.Label(main, text="Power Source:").grid(row=1, column=0, sticky=W)
        self.v_power = tkinter.StringVar()
        tkinter.ttk.Combobox(main, textvariable=self.v_power, values=sorted(self.show_obj.all_power_sources), state='readonly').grid(row=1, column=1, sticky=W)

        tkinter.ttk.Label(main, text="Network:").grid(row=2, column=0, sticky=W)
        self.v_net = tkinter.StringVar()
        tkinter.ttk.Combobox(main, textvariable=self.v_net, values=sorted(self.show_obj.networks), state='readonly').grid(row=2, column=1, sticky=W)

        for row, field_name in enumerate([n for n in sorted(self.field_spec) if not n.startswith('_')]):
            tkinter.ttk.Label(main, text=field_name+":").grid(row=row+3, column=0, sticky=W)
            print(field_name)
            print(self.field_spec[field_name])
        ...
        tkinter.ttk.Button(main, text="Remove This Controller", state='normal' if self.ctl_obj is not None else 'disabled', command=self.delete_me).grid(row=0, column=10)
        self.pack(side=TOP, expand=True, fill=BOTH)
    ...
    def _save(self):
        new_id = self.v_id.get()
        if new_id == '':
            tkinter.messagebox.showerror("Invalid ID", "Each controller must have an ID.")
            return

        if new_id in self.show_obj.controllers and self.show_obj.controllers[new_id] is not self.ctl_obj:
            tkinter.messagebox.showerror("Duplicate ID", 'The ID "{0}" is already assigned to another controller.'.format(new_id))
            return

        net_id = self.v_net.get()
        if net_id not in self.show_obj.networks:
            tkinter.messagebox.showerror("Missing Network", 'Each controller must be assigned a communications network.')
            return

        pwr_id = self.v_power.get()
        if pwr_id not in self.show_obj.all_power_sources:
            tkinter.messagebox.showerror("Missing Power Source", 'Each controller must be assigned a power source.')
            return
        ...
        if self.ctl_obj is None:
            try:
                ...
                tkinter.messagebox.showerror("Error", "Can't create that controller: {0}".format(err))
                return
            else:
            ...
            try:
                ...
                tkinter.messagebox.showerror("Error in controller values", "{0}".format(err))
                return
        ...
        def save_file(self):
            if self.file_name is None:
                ...
                try:
                    ...
                    tkinter.messagebox.showerror("Error Saving Configuration Profile", "Error writing {1}: {0}".format(traceback.format_exc(0), self.file_name))
                else:
                    self.modified_since_saved = False
