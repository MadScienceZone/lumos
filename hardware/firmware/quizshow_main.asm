...
CB_IDX	SET	0
	...
	WHILE	CB_IDX < N_BUTTONS
	 CLRF	BTN_X0_TIME_T+#v(CB_IDX), BANKED
	 CLRF	BTN_X0_TIME_U+#v(CB_IDX), BANKED
	 CLRF	BTN_X0_TIME_H+#v(CB_IDX), BANKED
	 CLRF	BTN_X0_TIME_L+#v(CB_IDX), BANKED
	 IF FULL_RESET
	  CLRF	BTN_X0_FLAGS+#v(CB_IDX), BANKED
	  CLRF	BTN_X0_LOCKTMR+#v(CB_IDX), BANKED
	  CLRF	BTN_X0_DEB_TMR+#v(CB_IDX), BANKED
	 ELSE
	  BCF	BTN_X0_FLAGS+#v(CB_IDX), BTN_FLG_PRESSED, BANKED
	 ENDIF
CB_IDX	 ++
	ENDW
	ENDM
...
SCAN_DEBOUNCE MACRO BTN_IDX, BTN_PORT, BTN_BIT, LT, LTN, LCK
	...
	; this flag state.
	;
	BCF	BTN_X0_FLAGS+#v(BTN_IDX), BTN_FLG_ACTIVE, BANKED
...
SCN_DB_DRAIN_#v(SCN_DBN):
	...
	;
	BANKSEL	QUIZSHOW_DATA
	DECF	BTN_X0_DEB_TMR+#v(BTN_IDX), F, BANKED
	BRA	SCN_DB_DONE_#v(SCN_DBN)
...
SCN_DB_RST_LCK_#v(SCN_DBN):
	...
	;
	BANKSEL	QUIZSHOW_DATA
	BSF	BTN_X0_FLAGS+#v(BTN_IDX), BTN_FLG_LOCKED, BANKED
	MOVFF	QUIZSHOW_LCKTM, BTN_X0_LOCKTMR+#v(BTN_IDX)
...
SCN_DB_DOWN_#v(SCN_DBN):
	...
	;
	BANKSEL	QUIZSHOW_DATA
	INCFSZ	BTN_X0_DEB_TMR+#v(BTN_IDX), F, BANKED
	BRA	SCN_DB_DONE_#v(SCN_DBN)
	;
	...
	; officially down.
	;
	SETF	BTN_X0_DEB_TMR+#v(BTN_IDX), BANKED
	BTFSC	BTN_X0_FLAGS+#v(BTN_IDX), BTN_FLG_LOCKED, BANKED	; is it locked?
	BRA	SCN_DB_RST_LCK_#v(SCN_DBN)			; stay locked longer
	BTFSC	BTN_X0_FLAGS+#v(BTN_IDX), BTN_FLG_ACTIVE, BANKED	; if we already saw the button press
	BRA	SCN_DB_DONE_#v(SCN_DBN)				;   then we don't need to do anything more
	;
	; We didn't know the button was down yet. Let's see what
	; that changes...
	;
	BSF	BTN_X0_FLAGS+#v(BTN_IDX), BTN_FLG_ACTIVE, BANKED	; mark as "down"
	BTFSC	BTN_X0_FLAGS+#v(BTN_IDX), BTN_FLG_MASKED, BANKED	; is it masked?
	BRA	SCN_DB_DONE_#v(SCN_DBN)				; then ignore it
	BTFSC	BTN_X0_FLAGS+#v(BTN_IDX), BTN_FLG_PRESSED, BANKED	; already noted?
	...
	;
	BSF	BTN_X0_FLAGS+#v(BTN_IDX), BTN_FLG_PRESSED, BANKED
	BCF	INTCON, 7, ACCESS				; disable interrupts
	BCF	INTCON, 6, ACCESS				; disable interrupts
	MOVFF	QS_BTN_TMR_U, BTN_X0_TIME_U+#v(BTN_IDX)
	MOVFF	QS_BTN_TMR_T, BTN_X0_TIME_T+#v(BTN_IDX)
	MOVFF	TMR1L, BTN_X0_TIME_L+#v(BTN_IDX)
	MOVFF	TMR1H, BTN_X0_TIME_H+#v(BTN_IDX)
...
SCN_DB_DONE_#v(SCN_DBN):
	...
	;
	BANKSEL	QUIZSHOW_DATA
	BTFSC	BTN_X0_FLAGS+#v(BTN_IDX), BTN_FLG_LOCKED, BANKED
	DECFSZ	BTN_X0_LOCKTMR+#v(BTN_IDX), F, BANKED
	BRA	SCN_DB_END_#v(SCN_DBN)
	BCF	BTN_X0_FLAGS+#v(BTN_IDX), BTN_FLG_LOCKED, BANKED	; release the lock
	BCF	BTN_X0_FLAGS+#v(BTN_IDX), BTN_FLG_LCK_ACK, BANKED
	LOCAL_LIGHT_INDICATOR BTN_IDX, LT, LTN, LCK
...
;	
;	When the scanner starts, enabled buttons are illuminated white, disabled are off.
;	Anytime a button is locked, it is illuminated in flashing red.
; 
;
LLIM_N	SET	0
LOCAL_LIGHT_INDICATOR MACRO BTN_IDX, LT, LTN, LCK
	BANKSEL	QUIZSHOW_DATA
	BTFSS	BTN_X0_FLAGS+#v(BTN_IDX), BTN_FLG_LOCKED, BANKED
	BRA	LLIM_NOT_LOCKED_#v(LLIM_N)
	BTFSC	BTN_X0_FLAGS+#v(BTN_IDX), BTN_FLG_LCK_ACK, BANKED
	BRA	LLIM_NOT_LOCKED_#v(LLIM_N)
	;
	; light up for locked mode for the first time
	;
	BSF	BTN_X0_FLAGS+#v(BTN_IDX), BTN_FLG_LCK_ACK, BANKED	; don't do it again
LLIM_LTN SET	LTN
	WHILE LLIM_LTN > 0
LLIM_LTN --
...
LLIM_NOT_LOCKED_#v(LLIM_N):
	BANKSEL	QUIZSHOW_DATA
	BTFSC	BTN_X0_FLAGS+#v(BTN_IDX), BTN_FLG_MASKED, BANKED
	BRA	LLIM_OFF_#v(LLIM_N)
	BTFSC	BTN_X0_FLAGS+#v(BTN_IDX), BTN_FLG_PRESSED, BANKED
	BRA	LLIM_OFF_#v(LLIM_N)
...
; if standalone mode:
;	if L4 pressed:
;		reset all buttons and lights, stop scanner
;	if X4 pressed:
;		start scanning
;	if anything else pressed:
;		light up X4 to indicate player who was in first
;		set flag to ignore further results (or, I suppose, keep re-comparing them :)
;	
