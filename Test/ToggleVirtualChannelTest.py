...
class ToggleVirtualChannelTest (unittest.TestCase):
    def setUp(self):
        self.c = DummyChannelObject('spam')

    def testCons(self):
        vc = ToggleVirtualChannel('id', self.c, 'name', '#ffffff')
        self.assertEqual(vc.id, 'id')
        self.assertEqual(vc.channel, self.c)
        self.assertEqual(vc.name, 'name')
        self.assertEqual(vc.color, '#ffffff')
        self.assertEqual(vc.type, 'toggle')

        vc = ToggleVirtualChannel('id', self.c, color='#ffffff')
        self.assertEqual(vc.id, 'id')
        self.assertEqual(vc.channel, self.c)
        ...
        vc = ToggleVirtualChannel('id', self.c)
        self.assertEqual(vc.id, 'id')
        self.assertEqual(vc.channel, self.c)
        self.assertEqual(vc.name, 'spam')
        self.assertEqual(vc.color, '#ffffff')
        ...
        self.assertRaises(ValueError, ToggleVirtualChannel, 'id', (self.c, self.c))
        self.assertRaises(ValueError, ToggleVirtualChannel, 'id', self.c, color='13#57')
    ...
    def test_factory(self):
        vc = virtual_channel_factory('toggle', id='thisOne', channel=self.c, name='This One', color='#123456')
        self.assertEqual(vc.type, 'toggle')
