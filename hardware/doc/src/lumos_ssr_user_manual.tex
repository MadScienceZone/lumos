...
If \Var*{s}=1, the scanner is stopped. Otherwise it will continue running after
...
%	& \bitbox{7}{\Var*{post}}\\
  ...
  \wordbox[]{1}{$\vdots$ \\[1ex]} 
  \end{rightwordgroup}\\
	\wordbox{1}{\Var*{fault$_1$}}\\
	\wordbox{1}{\Var*{fault$_0$}}\\
%  \bitbox{1}{\z0} 
...
\begin{description}
	...
	\item[General Status:]
		...
		is contained in fields \Var*{fault$_1$} and \Var*{fault$_0$}. {\color{red}In Protocol~0,
...
%			\bitbox[br]{1}{\z0} \\
	...
	\begin{BF}
		...
		\bitbox{1}{\z0} &
			...
			\bitbox[br]{1}{\z0} \\
	\end{BF}
	Sets the button masks for player \Var*{unit}. For each button
	...
	\begin{BF}
		...
		\bitbox{1}{\z0} & \bitbox{3}{\color{lightgray}\rule{\width}{\height}} 
			& \bitbox{4}{\Var*{paddr}}\\
		\bitbox{1}{\z0} & \bitbox{7}{\Var*{pchannel}}\\
		\wordbox[]{1}{$\vdots$ \\[1ex]}
		\end{rightwordgroup}\\
		\bitbox{1}{\z0} &
			\bitbox[tb]{1}{\z0} &
	...
	All other players will be assumed to have no corresponding Lumos device
	for their lights (possibly because they are directly controlled by the
	\acronym{QSMC} itself). Any player positions which are in the first
	\Var*{n} but which do belong to the controller should be assigned the
	controller's address.

	The player's light which will be used to indicate lock-out condition will
	be channel \Var*{pchannel} on device address \Var*{paddr}. 
	If \Var*{pchannel} is specified as 127 (\z{01111111}), then it is assumed
	that the normal default light channel will be used, as summarized in
	Table~\ref{tbl:defledchan}.
\begin{table}
	\begin{QS*}
	\begin{center}
		\begin{tabular}{rrl}\toprule
			\bfseries Player & \bfseries Default Channel \\\midrule
			 0 & 0 & (\acronym{QSCC} units)\\
			 1 & 0\\
			 2 & 0\\
			 3 & 0\\
			 4 & 0\\
			...
			\bottomrule
		\end{tabular}
		\caption{Default Player Lock-Out Channels\label{tbl:defledchan}}
	\end{center}
	\end{QS*}
\end{table}
\begin{table}
	\begin{QS*}
		\begin{center}
			\begin{tabular}{rll}\\\toprule
				...
				0 & player 8 X red & X red \\
				1 & player 8 X green & X green \\
				2 & player 8 X blue & X blue \\
				3 & player 8 L light & L red \\
				4 & player 9 X red & L green \\
				5 & player 9 X green & L yellow \\
				6 & player 9 X blue & nameplate red \\
				7 & player 9 L light & nameplate green \\
				8 & player 10 X red & nameplate blue \\
				9 & player 10 X green & A light \\
				10& player 10 X blue & B light \\
				11& player 10 L light & C light \\
				12& player 11 X red & D light \\
				13& player 11 X green & spare output \\
				14& player 11 X blue \\
				15& player 11 L light \\
				16& player 12 X red \\
				17& player 12 X green \\
				18& player 12 X blue \\
				19& player 12 L light \\
				20& player 13 X red \\
				21& player 13 X green \\
				22& player 13 X blue \\
				23& player 13 L light \\
				24& player 14 X red \\
				25& player 14 X green \\
				26& player 14 X blue \\
				27& player 14 L light \\
				28& player 15 X red \\
				29& player 15 X green \\
				30& player 15 X blue \\
				31& player 15 L light \\
				32& spare output 38 \\
				33& spare output 39 \\
				34& spare output 40 \\\bottomrule
			\end{tabular}
...
\end{table}
	...
	For example, for a 16-player \acronym{QSMC} where the first 8 positions
	are on external Lumos-compatible stations and the last 8 are directly 
	controlled, setting \Var*{n}=0 means you will only be using the direct
	player positions (\#8--\#15), or that you don't want any Lumos light
	feedback to the players during scans.

	If you use three players with remote stations, you would set \Var*{n}=3 and
	give the Lumos addresses and channel numbers for players \#0--\#2.  Then no 
	lighting feedback will be given for players \#3--\#7, but the directly 
	controlled players \#8--\#15 will get normal direct feedback.
...
shown as \Var{a} and so on to \Var{x} as the rightmost character. If the corresponding
...
{\bfseries This command appeared staring with Protocol 1.}
...
\begin{QS*}
	\z{0x70a}--\z{0x70d} are used for QS-specific commands.
...
\subsection{\z{0x71b} [reply] Button Query Full Status Response}
	\begin{BF}
		...
		\bitbox{1}{\z0} & \bitbox{1}{\Var{s}} & \bitbox{1}{\z1}
			& \bitbox{5}{\Var*{n}} \\
		\begin{rightwordgroup}{Repeat \Var*{n} times}
	...
	button scan to the time the button was pressed. If all bits of a timestamp
	are zero, the button has not yet been pressed. If all bits are ones, the
	button is masked out.

	(If a button was somehow miraculously pressed in less than a microsecond,
	it will be reported as 1 to distinguish it from an unpressed button; any
	...
	\z{0xFFFE} to distinguish it from a masked-out button.)
...
\subsection{\z{0x71c} [reply] Button Query ``Ping'' Response}
	\begin{BF}
		...
		\bitbox{1}{\z0} & \bitbox{1}{\Var{s}} & \bitbox{1}{\z0}
			& \bitbox{5}{\Var*{n}} \\
		\begin{rightwordgroup}{Repeat \Var*{n} times}
