...
This circuit design, including but not limited to any associated plans, schematics, designs, board layouts, documentation, and/or components, is \mc{EXPERIMENTAL} and for \mc{EDUCATIONAL} purposes only. It is not a finished consumer-grade product and has not been reviewed by a body such as Underwriter's Laboratories. It is assumed that you have the necessary understanding and skill to assemble and/or use electronic circuits.
...
as shipped to you, but are not liable for damage caused by your handling or assembly of the unit.
...
\begin{itemize}
	...
	\item	{\bfseries \ix{RS-485} \ix{terminator plug}.}  \emph{(RS-485 models only.)}
		...
		Usually, your RS-485 converter will have a terinator built into it, so it can form one end of the chain
		by itself.
...
\begin{enumerate}
	...
	\item	Refer to Figure~\ref{fig:netcables} to see how the serial network cables look
		...
		from the PS's RS-485 converter.  The orange cable is going out from this Lumos controller to the next one in the
...
\chapter{Appendix A: Connector Pinouts}
...
\begin{tikzpicture}[scale=.25]
	...
	\node [below] at (3,8) {1};
	\node [below] at (10,8) {8};
	\node [right] at (15,12) {\strut 1 Return Data ($+$); \emph{[Full-duplex models only]}};
	\node [right] at (15,10.5) {\strut 2 Return Data ($-$); \emph{[Full-duplex models only]}};
	\node [right] at (15,9) {\strut 3 Cable Check};
	\node [right] at (15,7.5)  {\strut 4 Data ($-$)};
	\node [right] at (15,6)  {\strut 5 Data ($+$)};
	\node [right] at (15,4.5)  {\strut 6 Cable Check};
...
board which fits inside the modular plug, making a self-contained terminator plug.
\begin{center}
	\begin{circuitikz}
...
can be configured to react to the inputs as active-high or active-low.
...
fully on or fully off, the signal will pulse with a duty cycle corresponding to the dimmer level.
...
\section{Voltage Select Headers (J6--J8, J17)}
\begin{center}
	\begin{tikzpicture}[scale=.5]
...
\acronym{IDC} ribbon cable header.  This is used to connect the relays to the controlling circuit (e.g.,
...
\chapter{Appendix B: Troubleshooting}\label{ch:troubleshooting}
...
\end{tabular}
\end{center}
and the others who offered moral support, other contributions, or who wished to contribute
anonymously.
\section*{Technical Legacy}
...
Christmas and Do It Yourself Christmas, respectively).  I am pleased to contribute back my own
innovations on these themes (all of the rest of the Lumos boards, including control logic
and firmware, are my original contributions) for the benefit of the community.  
