...
def packet(id, op):
	if op in ('up','dn'):
		b = bits[id[0]] | bits[op]
		#print "packet(%s,%s): b=bits[%s] | bits[%s] (%x|%x=%x)" % (id, op, id[0], op, bits[id[0]], bits[op], b)
	else:
		b = bits[id[0]] | bits[id[1:]] | bits[op]
		#print "packet(%s,%s): b=bits[%s] | bits[%s] | bits[%s] (%x|%x|%x=%x)" % (id, op, id[0], id[1:], op, bits[id[0]], bits[id[1:]], bits[op], b)
		
	return chr(0xd5)+chr(0xaa)+chr((b>>8)&0xff)+chr(b&0xff)+chr(0xad)

class FirecrackerX10ControllerUnit (X10ControllerUnit):
	def __init__(self, power, resolution=21):
		'''
