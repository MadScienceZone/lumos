...
\begin{QS*}
	...
	of player positions supported by the hardware. Then there are \Var*{nplayers} sets of three bytes
	giving, respectively, the current button masks for that player, \Var*{paddr} (the Lumos device address
	for that player, and \Var*{pchannel} (the Lumos channel number of the \acronym{LED} to flash if that player gets
	locked out).
\end{QS*}
...
%			\bitbox[br]{1}{\z0} \\
	...
	Table~\ref{tbl:defledchan}. For players directly managed by the \acronym{QSMC},
	the Lumos device defaults to the \acronym{QSMC}'s device (i.e., if \Var*{pchannel}=127,
	this is the default for \Var*{paddr}). Otherwise, an explicit \Var*{paddr} is
	required.
\begin{table}
	\begin{QS*}
	\begin{center}
